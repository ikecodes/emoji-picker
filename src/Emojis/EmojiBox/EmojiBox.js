...
const EmojiBox = ({ emojiList, category, handleCategory, changeValue, handleChangeValue }) => {
  let emojis;
  ...
  return (
    <div className={styles.zcembemojibox}>
      ...
      <EmojiSearch category={category} emojiList={emojiList} handleCategory={handleCategory} category={category} changeValue={changeValue} handleChangeValue={handleChangeValue} />
      <p className={styles.zcembemojiboxtext}>{category}</p>
      <ul className={styles.zcembemojiboxlist}>{emojis && emojis.map((emoji) => <EmojiItem key={emoji.slug} emoji={emoji} />)}</ul>
    </div>
  );
};
